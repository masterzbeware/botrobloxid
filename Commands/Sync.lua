...
return {
  Execute = function()
      ...
      local function sendSyncCommand(targetName)
          local success, err = pcall(function()
              ...
              -- Kirim perintah sync
              commandHandler:InvokeServer("sync", targetName)
          end)
          if not success then
      ...
      local function handleCommand(msg, sender)
          ...
          if Admin:IsAdmin(sender) then
              if msg:sub(1,5) == "!sync" then
                  local targetName = msg:sub(7) -- Ambil nama setelah !sync 
                  if targetName and targetName ~= "" then
                      sendSyncCommand(targetName)
                  end
